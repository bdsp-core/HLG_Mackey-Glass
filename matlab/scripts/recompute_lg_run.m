...
% recompute_lg_run - Recompute loop gain (LG_min) from existing EM output.
%
%   T = recompute_lg_run(study_num, dataset, input_folder, run, version)
%
% Inputs:
%   study_num    - Study index/number
%   dataset      - Dataset/cohort name
%   input_folder - Path to input CSV files (with existing EM results)
%   run          - "series" or "parallel"
%   version      - "smooth" or "non-smooth"
%
% Outputs:
%   T - Table with LG_min_rem and LG_min_nrem added/updated
%
...
addpath('../utils');
    ...
    for rr = 1:2
        ...
        for s = 1:length(starts)
            ...
            LG_min = fcn_get_loop_gain(0.05, G_est, u_min);

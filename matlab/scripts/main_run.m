function T = main_run(study_num, dataset, input_folder, run, version)
% main_run - Run EM algorithm on a single sleep study.
%
%   T = main_run(study_num, dataset, input_folder, run, version)
%
% Inputs:
%   study_num    - Study index/number
%   dataset      - Dataset/cohort name
%   input_folder - Path to input CSV files
%   run          - "series" or "parallel"
%   version      - "smooth" or "non-smooth"
%
% Outputs:
%   T - Table with EM estimation results (D, L, Alpha, LG, G for NREM/REM)
%
...
addpath('../utils');
    ...
    for rr = 1:2
        ...
        for s = 1:length(starts)
            ...
            [upAlpha, upgamma, uptau, Vo_est, h, u_avg] = fcn_em_algorithm_real_data(T_seg, w, L, gamma_init, tau_init, version);
            ...
            LG_est = fcn_get_loop_gain(L_est, G_est, u_avg);
